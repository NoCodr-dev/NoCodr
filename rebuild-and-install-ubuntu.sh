...
################################################################################
# NoCoder Extension - Ubuntu Build, Version Update & Installation Script
#
# This script is tailored for Ubuntu/Linux environments. It:
# 1. Ensures Node.js and pnpm are available (via Corepack)
# 2. Updates version in package.json files (Linux-compatible sed)
# 3. Cleans previous builds
# 4. Builds new VSIX package
# 5. Uninstalls old version from VS Code
# 6. Installs new version
# 7. Commits and pushes changes (optional)
#
# Usage:
#   ./rebuild-and-install-ubuntu.sh [version] [--skip-git]
#
# Examples:
#   ./rebuild-and-install-ubuntu.sh 1.0.1
#   ./rebuild-and-install-ubuntu.sh 1.0.2 --skip-git
#
# Notes:
# - Requires Node.js >= 20 and pnpm (activated via Corepack)
# - VS Code CLI 'code' must be in PATH (install via apt/snap)
################################################################################
...
# Update version in package.json file (Linux-compatible sed)
...
check_vscode_cli() {
  if ! command -v code &> /dev/null; then
    ...
    print_info "Install VS Code and ensure 'code' is in PATH. On Ubuntu:"
    echo "  - Snap:   sudo snap install code --classic"
    echo "  - Debian: https://code.visualstudio.com/docs/setup/linux"
    echo "Once installed, verify with: code --version"
    exit 1
  fi
}

# Setup Node.js and pnpm via Corepack
setup_node_env() {
  if ! command -v node &> /dev/null; then
    print_error "Node.js not found in PATH"
    print_info "Install Node.js 20.x (recommended):"
  ...
  if (( NODE_MAJOR < REQUIRED_NODE_MAJOR )); then
    print_warning "Detected Node.js v$NODE_VERSION. Version >= $REQUIRED_NODE_MAJOR is recommended."
  fi

  # Enable and activate pnpm via Corepack
  if ! command -v corepack &> /dev/null; then
    print_error "Corepack not found (should be bundled with Node >= 16)."
    print_info "Update Node.js or install pnpm manually: https://pnpm.io/installation"
    exit 1
  fi

  print_info "Enabling Corepack and activating pnpm@$REQUIRED_PNPM..."
  corepack enable || true
  corepack prepare pnpm@"$REQUIRED_PNPM" --activate

  if ! command -v pnpm &> /dev/null; then
    print_error "pnpm not available after Corepack activation"
    print_info "Try installing pnpm manually: curl -fsSL https://get.pnpm.io/install.sh | sh -"
    exit 1
  fi

  local PNPM_VERSION
  PNPM_VERSION=$(pnpm --version)
  print_success "Node.js: v$NODE_VERSION | pnpm: $PNPM_VERSION"
}
...
echo "╔═══════════════════════════════════════════════════════════════╗"
echo "║                                                               ║"
echo "║    NoCoder Extension - Ubuntu Build & Install Script         ║"
echo "║                                                               ║"
echo "╚═══════════════════════════════════════════════════════════════╝"
echo -e "${NC}\n"
...
echo ""
print_success "All done on Ubuntu! 🚀"
