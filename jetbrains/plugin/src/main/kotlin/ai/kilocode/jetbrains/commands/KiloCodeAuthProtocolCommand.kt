...
/**
 * JetBrains Protocol Command for handling NoCodr authentication URLs
 *
...
class KiloCodeAuthProtocolCommand : JBProtocolCommand("ai.kilocode.jetbrains.auth") {
    ...
    override suspend fun execute(target: String?, parameters: Map<String, String>, fragment: String?): String? {
        logger.info("Handling NoCodr auth protocol command: target=$target, parameters=$parameters")
        ...
        return try {
            ...
            val errorMsg = "Error handling NoCodr auth protocol command: ${e.message}"
